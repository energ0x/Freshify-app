...
const ROADMAP_KEYS = [
  { id: '1', level: 1,   titleKey: 'leagues.item1Title', descKey: 'leagues.item1Desc', icon: 'leaf',       color: '#2ECC71', isLeague: true },
  { id: '2', level: 10,  titleKey: 'leagues.item2Title', descKey: 'leagues.item2Desc', icon: 'star',       color: '#27AE60', isLeague: false },
  { id: '3', level: 50,  titleKey: 'leagues.item3Title', descKey: 'leagues.item3Desc', icon: 'trending-up',color: '#229954', isLeague: false },
  { id: '4', level: 101, titleKey: 'leagues.item4Title', descKey: 'leagues.item4Desc', icon: 'ribbon',     color: '#3498DB', isLeague: true },
  { id: '5', level: 150, titleKey: 'leagues.item5Title', descKey: 'leagues.item5Desc', icon: 'restaurant', color: '#2980B9', isLeague: false },
  { id: '6', level: 201, titleKey: 'leagues.item6Title', descKey: 'leagues.item6Desc', icon: 'planet',     color: '#F1C40F', isLeague: true },
  { id: '7', level: 300, titleKey: 'leagues.item7Title', descKey: 'leagues.item7Desc', icon: 'trophy',     color: '#F39C12', isLeague: false },
];
...
export default function LeaguesScreen({ navigation }) {
  ...
  const currentLevelIndex = ROADMAP_KEYS.reduce((acc, curr, index) => {
  ...
  const renderItem = ({ item, index }) => {
    ...
    const isLast = index === ROADMAP_KEYS.length - 1;
          ...
          <View style={styles.cardHeader}>
            ...
            <View style={[styles.levelBadge, { backgroundColor: isUnlocked ? `${item.color}15` : COLORS.surfaceVariant }]}>
              <Text style={[styles.levelBadgeText, { color: isUnlocked ? item.color : COLORS.textLight }]}>
                {t('leagues.levelBadge', { level: item.level })}
              </Text>
            </View>
          </View>
          <Text style={[styles.cardDesc, { color: COLORS.textLight }]}>{t(item.descKey)}</Text>
        </View>
      </View>
    );
  };
    ...
    <View style={[styles.container, { backgroundColor: COLORS.background }]}>
      ...
      <View style={[styles.header, { backgroundColor: COLORS.surface, paddingTop: insets.top + 10 }]}>
        <View style={styles.headerTop}>
          ...
          <Text style={[styles.headerTitle, { color: COLORS.text }]}>{t('leagues.title')}</Text>
          <View style={{ width: 24 }} />
        </View>
        ...
        <View style={styles.myStatsRow}>
          ...
          <View style={styles.statPill}>
            ...
            <Text style={[styles.statPillText, { color: COLORS.text }]}>{t('leagues.myLevel', { level: currentLevel })}</Text>
          </View>
        </View>
      </View>
      ...
      >
        ...
        <Text style={styles.fabText}>{t('leagues.goToMyLevel')}</Text>
      </TouchableOpacity>
    </View>
  );
}
